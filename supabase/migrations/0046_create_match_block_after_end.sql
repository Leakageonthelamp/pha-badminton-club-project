-- Block new match assignment after scheduled end or early end.
...
begin
	...
	if v_session.ended_early or now() >= v_session.end_at then
		raise exception 'Session has ended — no new matches can be assigned';
	end if;
